                        ...
                        CASE
                            WHEN rp.status_lanjut = 'Ranap' THEN
                                (
                                    ...
                                    WHERE rpr.no_rawat = rp.no_rawat
                                )
                            WHEN rp.status_lanjut = 'Ralan' THEN
                                (
                                    SELECT COUNT(*)
                                    FROM resume_pasien rpj
                                    WHERE rpj.no_rawat = rp.no_rawat
                                )
                            ELSE 0
                        END > 0 as ada_resume
                    "),
                ...
                $q->where(function ($x) use ($search) {
                    ...
                    $x->where('rp.no_rawat', 'like', "%{$search}%")
                        ...
                        ->orWhereExists(function ($sub) use ($search) {
                            $sub->select(DB::raw(1))
                                ->from('dpjp_ranap as dr')
                                ->join('dokter as dd', 'dd.kd_dokter', '=', 'dr.kd_dokter')
                                ->whereColumn('dr.no_rawat', 'rp.no_rawat')
                                ->where('dd.nm_dokter', 'like', "%{$search}%");
                        });
                });
            })
